...
public class OddballMecBot {
    ...
    public void setDrivePowerDiagonal(float pxPrime, float pyPrime, float pTheta){
        final float invSqrt2 = 1.0f/(float)Math.sqrt(2);
        float px = (pxPrime - pyPrime) * invSqrt2;
        float py = (pxPrime + pyPrime) * invSqrt2;
        setDrivePower(px, py, pTheta);
    }
    ...
    public Pose updateOdometry(){
       ...
       float dYR = 0.25f * (lNew + rNew - fNew + bNew) / TICS_PER_INCH;
       float dXR = 0.25f * (-lNew + rNew + fNew + bNew) / TICS_PER_INCH;
