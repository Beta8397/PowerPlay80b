...
import org.firstinspires.ftc.teamcode.omnibot.OddballMecBot;
import org.firstinspires.ftc.teamcode.omnibot.OmniBot;

@TeleOp(name = "OddballBotDiagnostics", group = "Test")
//@Disabled
public class OddballMecBotDiagnostics extends LinearOpMode {

    private OddballMecBot bot = new OddballMecBot();
...
//        bot.getBackRight().setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
    ...
    protected void doOneIteration(){
        ...
        if (gamepad1.a) handleMotorPowers(0.2f,0,0,0);
            ...
            float px = gamepad1.left_stick_x / 2;
            float py = -gamepad1.left_stick_y / 2;
            float pa = -gamepad1.right_stick_x / 2;
    ...
    private void handleBotPowers(float px, float py, float pa){
        bot.setDrivePowerDiagonal(px, py, pa);
