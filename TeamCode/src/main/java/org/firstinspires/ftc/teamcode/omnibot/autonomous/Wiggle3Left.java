...
@Autonomous(name = "Wiggle3Left")
public class Wiggle3Left extends OmniBotAuto {
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        driveToPosition(midSpeed, 36, 93, -90, 1, null);

        bot.setPivotPosition(OmniBot.PIVOT_SCORING);
        sleep(400);
        driveToPosition(lowSpeed, 38, 92f, -90, 1, null);
        ...
        driveToPosition(ultraHighSpeed, 35,128,-90,1,
        ...
        driveLine( -90, new VectorF(36,128), 0,
                highSpeed, 2, 22.5f,
         ...
         * and grab cone
         */
        driveToPosition(highSpeed, 57.5f, 124.5f, 0, 1, null);
        turnToHeading(-135, 3, 6, 150);
        bot.openClaw();
        bot.setLiftPosition(-440);
        sleep(200);
        driveToPosition(highSpeed, 57.5f, 130.5f, -135, 1, null);
        bot.closeClaw();
        sleep(400);
        ...
        driveToPosition(highSpeed,  52.75f, 123.75f, 90, 1, null);
        ...
        * Pick up second cone from stack
        ...
        driveToPosition(highSpeed, 58.5f, 126, -135, 1, null);
        turnToHeading(90, 3, 6, 150);
        driveToPosition(highSpeed,  52.75f, 121.75f, 90, 1,
                new PowerPlayDistUpdater(Quadrant.RED_LEFT, HeadingIndex.H_90, false, true ));
        bot.setLiftPosition(OmniBot.LIFT_LOW + 200);
        sleep(200);
        bot.openClaw();
        sleep(200);
        ...
        switch(signalResult){
            case THREE:
                driveToPosition(highSpeed, bot.getPose().x-2, 126, -90, 1,null);
                break;
            case TWO:
//                adjustPositionColor(0.5f, 90, 4, 5, 0.1f);
...
//                bot.setPose(57.5f, bot.getPose().y, (float)Math.toDegrees(bot.getPose().theta));
                bot.setCovariance(new GeneralMatrixF(2,2, new float[]{
                        1, 0, 0, bot.getCovariance().get(1,1)}));
                driveToPosition(ultraHighSpeed, bot.getPose().x-2, 103, -90, 1,
                        new WiggleProfile(3, 0.1f, 2),
                        new PowerPlayDistUpdater(Quadrant.RED_LEFT, HeadingIndex.H_NEG_90, false, false));
                break;
            case ONE:
//                adjustPositionColor(0.5f, 90, 4, 5, 0.1f);
...
//                        new PowerPlayDistUpdater(Quadrant.BLUE_RIGHT, HeadingIndex.H_NEG_90, false, true));
                driveToPosition(lowSpeed, bot.getPose().x - 3.5f, 80, -90, 1,
